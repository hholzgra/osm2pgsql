...
table_pgsql_t::table_pgsql_t(const database_options_t &database_options, const string& name, const string& type, const columns_t& columns, const hstores_t& hstore_columns,
...
table_pgsql_t::table_pgsql_t(const table_pgsql_t &other) :
  table_t(other),
  sql_conn(nullptr),
  copyMode(false),
  buffer(), 
  copystr(other.copystr)
{
...
table_pgsql_t::~table_pgsql_t()
{
  teardown();
}

void table_pgsql_t::teardown()
{
    if(sql_conn != nullptr)
    {
        PQfinish(sql_conn);
...
void table_pgsql_t::begin()
{
    pgsql_exec_simple(sql_conn, PGRES_COMMAND_OK, "BEGIN");
}

void table_pgsql_t::commit()
{
    stop_copy();
...
void table_pgsql_t::connect()
{
    //connect
    PGconn* _conn = PQconnectdb(database_options.conninfo().c_str());
...
void table_pgsql_t::write_node(const osmid_t id, const taglist_t &tags, double lat, double lon)
{
    write_row(id, tags, (point_fmt % lon % lat).str());
}

void table_pgsql_t::delete_row(const osmid_t id)
{
    stop_copy();
    pgsql_exec_simple(sql_conn, PGRES_COMMAND_OK, (del_fmt % name % id).str());
}

void table_pgsql_t::write_row(const osmid_t id, const taglist_t &tags, const std::string &geom)
{
    //add the osm id
...
void table_pgsql_t::write_columns(const taglist_t &tags, string& values, std::vector<bool> *used)
{
    //for each column
    for(columns_t::const_iterator column = columns.begin(); column != columns.end(); ++column)
    {
...
void table_pgsql_t::write_tags_column(const taglist_t &tags, std::string& values,
...
void table_pgsql_t::write_hstore_columns(const taglist_t &tags, std::string& values)
{
...
void table_pgsql_t::escape4hstore(const char *src, string& dst)
{
    dst.push_back('"');
...
void table_pgsql_t::escape_type(const string &value, const string &type, string& dst) {
...
table_t::wkb_reader table_pgsql_t::get_wkb_reader(const osmid_t id)
{
    ...
    return table_t::wkb_reader(new wkb_reader_impl(res));
}
