...
{
    auto *clone = new output_sql_t(*this);
    clone->m_mid = cloned_middle;
    return std::shared_ptr<output_t>(clone);
}

output_sql_t::output_sql_t(const middle_query_t* mid, const options_t &o)
    : output_t(mid, o),
...
void output_sql_t::merge_pending_relations(output_t *other)
{
    auto opgsql = dynamic_cast<output_sql_t *>(other);
    if (opgsql) {
        osmid_t id;
...
void output_sql_t::merge_expire_trees(output_t *other)
{
    auto *opgsql = dynamic_cast<output_sql_t *>(other);
    if (opgsql)
